# Exp-LCB-A：Coder-1.5B + APPS SFT warmup + 纯 GRPO（LR=5e-6，无 OPD）
#
# 对照关系（和 MBPP Exp 15 类比）：
#   MBPP Exp 15：SFT warmup + 纯GRPO + LR=5e-6 → 0.4533 (+10.5%)
#   Exp-LCB-A：  同样配置，但在 APPS 上训练、LCB 上评测
#
...
#
# 启动（SFT warmup 完成后运行）：
#   nohup bash <repo>/scripts/run_apps_grpo_lr5e6.sh \
#     > <repo>/train_apps_grpo_lr5e6.log 2>&1 &
...
EXPERIMENT_NAME=coder_1_5b_apps_grpo_lr5e6
LOG_FILE=$HOME/codellmRL/train_apps_grpo_lr5e6.log
